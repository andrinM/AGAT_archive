...
def run_algorithms(members):
    ...
    csv_files = 'csv_files'
    file_path = os.path.join(csv_files, 'results_36_members.csv')
    results_df.to_csv(file_path, index=False)
    file_path = os.path.join(csv_files, 'avg_results_36_members.csv')
    avg_results_df.to_csv(file_path, index=False)
   

members = create_random_dataframe(n_rows = 36, hom = 2, het = 2, feature_range = 3, hot_het = 2, hot_hom = 2)
